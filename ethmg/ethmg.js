...
function get_buy_eth_info(val){
	...
	return new Promise(function(resolve){
		$.ajax({
			url:"https://api.dex.ag/trade?from=DAI&to=ETH&toAmount="+ String(val.toFixed(3)) +"&dex=best",
			dataType:"json",
		})
...
function get_sell_eth_info(val){
	...
	return new Promise(function(resolve){
		$.ajax({
			url:"https://api.dex.ag/trade?from=ETH&to=DAI&fromAmount="+ String(val.toFixed(3)) +"&dex=best",
			dataType:"json",
		})

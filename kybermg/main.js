...
function get_buy_eth_info(val){
	...
	return new Promise(function(resolve){
		$.ajax({
			url:"https://api.dex.ag/price?from=" + token + "&to=ETH&toAmount="+ String(val.toFixed(3)) +"&dex=kyber",
			dataType:"json",
		})
...
function get_sell_eth_info(val){
	...
	return new Promise(function(resolve){
		$.ajax({
			url:"https://api.dex.ag/price?from=ETH&to=" + token + "&fromAmount="+ String(val.toFixed(3)) +"&dex=kyber",
			dataType:"json",
		})

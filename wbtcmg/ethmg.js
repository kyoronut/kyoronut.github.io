...
function get_buy_wbtc_info(val){
	console.log(val);
	return new Promise(function(resolve){
		$.ajax({
			url:"https://api.dex.ag/trade?from=" + token + "&to=WBTC&toAmount="+ String(val.toFixed(3)) +"&dex=best",
			dataType:"json",
		})
...
function get_sell_wbtc_info(val){
	console.log(val);
	return new Promise(function(resolve){
		$.ajax({
			url:"https://api.dex.ag/trade?from=WBTC&to=" + token + "&fromAmount="+ String(val.toFixed(3)) +"&dex=best",
			dataType:"json",
		})
...
for(var i = 0; i < n; i++){
	list[i] = get_buy_wbtc_info(am[i]);
	list[i + n] = get_sell_wbtc_info(am[i]);
}
Promise.all(list)
	.then(function(data){
		for(var i = 0; i < n; i++){

const n = 5;
const am = [10, 100, 1000, 10000, 30000];
...
function get_buy_eth_info(val){
	...
	return new Promise(function(resolve){
		$.ajax({
			url:"https://api.dex.ag/trade?from=" + token + "&to=DAI&toAmount="+ String(val.toFixed(3)) +"&dex=best",
			dataType:"json",
		})
...
function get_sell_eth_info(val){
	...
	return new Promise(function(resolve){
		$.ajax({
			url:"https://api.dex.ag/trade?from=DAI&to=" + token + "&fromAmount="+ String(val.toFixed(3)) +"&dex=best",
			dataType:"json",
		})
